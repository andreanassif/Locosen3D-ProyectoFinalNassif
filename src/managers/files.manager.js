...
class FilesContainer {
    constructor(filename){
        this.filename = options.fileSystem.join(__dirname,"..",`files/${filename}`);
    ...
    async getById(id) {
        ...
        if (index === -1) {
            return logger.fatal({message:`Error: no se encontró el id ${id}`, error:true});
        } else {
    ...
    async updateById(info, id) {
        ...
        const index = objects.findIndex(element=> element.id === parseInt(id))
        if (index == -1) {
            return logger.error({message:`Error al actualizar: no se encontró el id ${id}`});
        } else {
    ...
    async deleteById(id) {
        ...
        const index = objects.findIndex(element=> element.id === parseInt(id));
        if (index == -1) {
            return logger.error({message:`Error al borrar: no se encontró el id ${id}`});
        }
